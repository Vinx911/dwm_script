...
export DWM_STATUS_TMPFILE="/tmp/dwm_status.temp"
touch $DWM_STATUS_TMPFILE

# 设置某个模块的状态 update cpu mem ...
update() {
    [ ! "$1" ] && refresh && return                                      # 当指定模块为空时 结束
    bash $DWM_PATH/statusbar/packages/$1.sh                     # 执行指定模块脚本
    shift 1; update $*                                                   # 递归调用
}
...
click() {
    [ ! "$1" ] && return                                                 # 未传递参数时 结束
    bash $DWM_PATH/statusbar/packages/$1.sh click $2            # 执行指定模块脚本
    update $1                                                            # 更新指定模块
    refresh                                                              # 刷新状态栏
}
...
refresh() {
    ...
    source $DWM_STATUS_TMPFILE                                           # 从 temp 文件中读取模块的状态
